...
def download_iShares():
    ...
    for product, url in pbar:
        ...
        try:
            ...
            if product in ["CNDX"]:
                ticker_df = [{"ticker": t, "name": n, "sector": s, "asset_class": asset_class, "weight": w, "isin": isin, }
                             for t, n, s, asset_class, w, isin in zip(data_map.get("ticker").get("formattedValue"),
